...
# store stubs.h, stubs.c, base.h
# check if dependency.h/dependency2.h exists and store it. If so, check if dependency.c exists and also store it.
# check if file contains "include "dependency2.h"". If so do the following checks with dependency2.h
# check if file contains "include "dependency.h"" then add dependency.h (and depdency.c if exists) to prompt.
# if file contains dependency.h, then check if dependency.h contains "include "../stubs.h"" then add stubs.h and stubs.c to prompt.

# if file contains "include "../stubs.h"" then add stubs.h and stubs.c to prompt
# if file contains "include "../base.h"" then add base.h
...
for folder in os.listdir(buggy_dir):
    if not folder.endswith(".c") and not folder.endswith(".h"):
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith(".c") and not filename == "dependency.c":
                file_path = os.path.join(folder_path, filename)
                output_name = folder + "." + filename + ".gpt_out.txt"
