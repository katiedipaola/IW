...
static int _bHeap_Compare(const BHEAP_VAL_TYPE *parent,
...
static void bHeap_Init(int (*compare)(const BHEAP_VAL_TYPE *parent,
                                      const BHEAP_VAL_TYPE *node)) {
  bHeap_Info.size = 0;
...
static void bHeap_UpHeap(const size_t nodeIdx) {
  ...
  const int compareResult = bHeap_Info.compare(&(bHeap_Node[parentIdx].val),
                                               &(bHeap_Node[nodeIdx].val));
...
static void bHeap_DownHeap(const size_t nodeIdx) {
  ...
  if (leftIdx < bHeap_Info.size) {
    const int leftResult = bHeap_Info.compare(&(bHeap_Node[nodeIdx].val),
                                              &(bHeap_Node[leftIdx].val));
    ...
    if (rightIdx < bHeap_Info.size) {
      const int rightResult = bHeap_Info.compare(&(bHeap_Node[nodeIdx].val),
                                                 &(bHeap_Node[rightIdx].val));
      ...
      if (swapTarget == BHEAP_INVALID_NODE) {
        ...
        if (0 < rightResult) {
          const int rightLeftResult = bHeap_Info.compare(
              &(bHeap_Node[leftIdx].val), &(bHeap_Node[rightIdx].val));
